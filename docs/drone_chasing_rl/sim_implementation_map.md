...
| 10 | INTERCEPT closing law | v_cmd = clip(1.0·(range−r_cap), 0, 1.2 m/s), on the same delayed measurement; r_cap = 0.5 m, B = +10, λ = 0.1, approach band 2–6 m | directly from sim_training_architecture §1; gains are config, recorded with results |
...
| 19 | Tier-B odometry sync | `GzTransportBackend.step()` waits for the world clock AND for every model's odometry stamp to reach the step boundary (the publisher is 100 Hz sim-time) | odometry arrives on its own topic; without the wait the real backend serves poses up to one publish period stale — invisible on the kinematic fake |
