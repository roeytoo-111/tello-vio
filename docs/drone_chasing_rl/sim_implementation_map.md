...
| 26 | First contact with real gz-sim 8.15 (2026-09-17) | Three defects the fake backend could not show, all fixed and re-measured: (a) `Node.request` never returns once the process subscribes to anything (the binding holds the GIL while blocking; the server still executes the call) → service calls go through a subscription-free helper subprocess; (b) apt `gz.msgs10` _pb2 files vs pip protobuf 7 → pure-Python protobuf selected before first import; (c) X3 rotor-drag coefficient copied to an 80 g vehicle gave ~36× the drag deceleration (yaw stalled at 24 % of command, forward at 13 %) → scaled by rotor speed and mass (now 91 % / 83 %). Result: **sign test PASS on real gz** (+yaw→+u, so gz issue #2657 does not affect this velocity-control path); lockstep lands exactly on +0.100 s sim time | `calibrate_lag` now measures sim T_lag 0.84 s forward / 0.54 s vertical / 0.66 s yaw vs the 0.25 s **placeholder** — deliberately NOT tuned until the Phase-1 Tello step-response flight gives the measured target |
